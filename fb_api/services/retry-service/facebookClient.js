...
async function hideComment(commentId) {
  ...
  try {
    ...
    if (result.body?.success === true || result.status === 200) {
      ...
      console.error(`[FacebookClient] ❌ Không thể ẩn comment ${commentId}:`, result.body);
      return false;
    }
  } catch (err) {
...
async function deleteComment(commentId) {
  ...
  try {
    ...
    if (result.body?.success === true || result.status === 200) {
      ...
      console.error(`[FacebookClient] ❌ Không thể xóa comment ${commentId}:`, result.body);
      return false;
    }
  } catch (err) {
...
async function replyComment(commentId, message) {
  ...
  try {
    ...
    if (result.body?.id || result.body?.success === true || result.status === 200) {
      console.log(`[FacebookClient] 💬 Đã reply comment ${commentId} thành công. ID: ${result.body?.id || "N/A"}`);
      return true;
    } else {
...
module.exports = { hideComment, deleteComment, replyComment };
